...
public class ActiveNotifier implements FineGrainedNotifier {
    ...
    private static final Logger logger = Logger.getLogger(SlakkListener.class.getName());

    SlakkNotifier notifier;

    public ActiveNotifier(SlakkNotifier notifier) {
        super();
        this.notifier = notifier;
    }

    private SlakkService getSlakk(AbstractBuild r) {
        AbstractProject<?, ?> project = r.getProject();
        String projectRoom = Util.fixEmpty(project.getProperty(SlakkNotifier.SlakkJobProperty.class).getRoom());
        return notifier.newSlakkService(projectRoom);
    }
    ...
    private void notifyStart(AbstractBuild build, String message) {
        getSlakk(build).publish(message, "green");
    }
    ...
    public void completed(AbstractBuild r) {
        AbstractProject<?, ?> project = r.getProject();
        SlakkNotifier.SlakkJobProperty jobProperty = project.getProperty(SlakkNotifier.SlakkJobProperty.class);
        ...
        if ((result == Result.ABORTED && jobProperty.getNotifyAborted())
            ...
            getSlakk(r).publish(getBuildStatusMessage(r), getBuildColor(r));
        }
    }
    ...
    public static class MessageBuilder {
        ...
        private SlakkNotifier notifier;
        private AbstractBuild build;

        public MessageBuilder(SlakkNotifier notifier, AbstractBuild build) {
            this.notifier = notifier;
